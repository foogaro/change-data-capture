...
RUN mkdir -p /opt/rh/plugins
RUN curl -O https://repo1.maven.org/maven2/io/debezium/debezium-connector-sqlserver/0.9.5.Final/debezium-connector-sqlserver-0.9.5.Final-plugin.tar.gz
RUN tar xvzf debezium-connector-sqlserver-0.9.5.Final-plugin.tar.gz -C /opt/rh/plugins
RUN rm -rf debezium-connector-sqlserver-0.9.5.Final-plugin.tar.gz
...
CMD ["config/connect-standalone.properties", "/opt/rh/plugins/InfinispanSinkConnector.properties"]
