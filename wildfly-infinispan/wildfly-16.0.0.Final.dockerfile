...
RUN mkdir -p /opt/rh
WORKDIR /opt/rh
RUN curl -O https://download.jboss.org/wildfly/16.0.0.Final/wildfly-16.0.0.Final.zip
RUN unzip wildfly-16.0.0.Final.zip
RUN rm -rf wildfly-16.0.0.Final.zip

RUN curl -O http://downloads.jboss.org/infinispan/9.4.11.Final/infinispan-wildfly-modules-9.4.11.Final.zip
RUN unzip infinispan-wildfly-modules-9.4.11.Final.zip
RUN rm -rf infinispan-wildfly-modules-9.4.11.Final.zip
RUN cp -a infinispan-wildfly-modules-9.4.11.Final/modules/* wildfly-16.0.0.Final/modules/
RUN rm -rf infinispan-wildfly-modules-9.4.11.Final

WORKDIR /opt/rh/wildfly-16.0.0.Final
...
RUN ./bin/jboss-cli.sh --file=deploy-cache-store.cli
RUN rm infinispan-listener-*.jar
RUN ./bin/jboss-cli.sh --file=local-caches-configuration.cli
RUN ./bin/jboss-cli.sh --file=local-caches.cli
RUN ls -la standalone/configuration
RUN cat standalone/configuration/standalone.xml
